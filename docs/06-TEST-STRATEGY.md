...
- `[用户确认 2026-08-13；2026-08-15、2026-08-28 修订]` 首页提供“组合分析”和“巴菲特框架顾问”两个独立入口与弹层。测试必须锁定组合分析点按即请求、顾问打开与草稿输入零请求、直接输入、非冒充/隐私披露、首次发送建立固定完整快照、最近六轮连续上下文、current-only 全字段白名单/身份与存储内部排除、语义十进制和真实 RFC 3339 时间戳、framework lens enum/数量/去重、一手基本面缺失时的 `EVIDENCE_GAP`、AI 行业/资产角色分类、本机 Decimal 暴露、六维体检、本机数字重绘、伪造外部归因/预测/直接交易指令拒绝、无持久化和失败不影响确定性详情。
