...
> 历史合同记录（此处的自动触发已由 2026-09-07 公开仓显式触发流程替代）：- `[用户确认 2026-08-13；2026-08-15、2026-08-28 修订]` 首页提供“组合分析”和“巴菲特框架顾问”两个独立入口与弹层。测试必须锁定组合分析点按即请求、顾问打开与草稿输入零请求、直接输入、非冒充/隐私披露、首次发送建立固定完整快照、最近六轮连续上下文、current-only 全字段白名单/身份与存储内部排除、语义十进制和真实 RFC 3339 时间戳、framework lens enum/数量/去重、一手基本面缺失时的 `EVIDENCE_GAP`、AI 行业/资产角色分类、本机 Decimal 暴露、六维体检、本机数字重绘、伪造外部归因/预测/直接交易指令拒绝、无持久化和失败不影响确定性详情。
...
- AI 完整快照 schema v4、高精度 Decimal/RFC 3339/合计/字段白名单、双券商现金分项、`INITIAL_ANALYSIS`/`CHAT` 模式、分析分类覆盖、本机行业/资产角色汇总、六类证据一致性、顾问空分类轻量回答、framework lenses/非冒充/证据缺口、固定 provider/分模式预算、多轮稳定快照、路由同源/大小/限流/无配置、客户端复验和两个独立弹层生命周期。
...
### 6.5 DeepSeek 组合分析与独立巴菲特框架顾问测试

contract 与快照生成测试验证：

- 请求只接受精确 schema v4 字段，current-only 组合摘要、逐只数据、总现金、IBKR/moomoo settled/pending、可选 IBKR 利息和合法历史必须往返保真；账户分项必须精确求和。CHAT/FOLLOW_UP 的 framework lenses 必须是一到三个唯一白名单值。身份、账号、设备、revision/savedAt、event id、历史库、草稿、备份和额外字段全部拒绝。
...
组件测试分别验证：两个首页入口打开不同 dialog；组合分析挂载零请求，首次显式开始使用最新 props；再验证紧凑加载/错误、行业/角色暴露、逐只分类/置信度、六个维度和本机 USD/CNY 证据；独立聊天打开、输入和关闭均为零请求，首次发送包含完整 `CHAT` 快照，后续只带最近六轮成功历史且父页刷新不替换固定快照，失败恢复草稿。两类弹层均验证不合规响应拒绝、确定性内容保留、发送边界和快照时间可见、数据变化只提示不自动请求、分析可主动重新解读、没有示例/建议问题或独立同意页、焦点陷阱、Escape 和关闭清除。测试只使用合成股票和金额，不做真实 DeepSeek 网络调用。
