...
→ 分别使用“组合分析”和“AI 对话”：前者点按即生成 DeepSeek 证据约束体检，后者打开零请求、发送才建立固定组合上下文并持续交流；数字仍来自本机，失败保留确定性详情
...
- `[用户确认 2026-08-13；2026-08-15 修订]` 首页提供“组合分析”和“AI 对话”两个独立入口；点按组合分析直接开始六维体检，聊天打开零请求、发送时才建立 current-only USD 固定快照。AI 推断行业/资产角色，本机 Decimal 汇总暴露并重绘数字；运行界面不再设明示同意步骤，缺少真源的高级风险指标明示不可计算，不提供目标价、收益保证、预测或直接交易指令；
...
- 在“更多操作”提供“数据安全与恢复”，并在首页总览与持仓表之间提供可见的“组合分析”和“AI 对话”两个入口；两者使用独立 dialog，组合分析保留确定性完整/部分状态，聊天打开时不请求；
...
- 组合分析点按后显示紧凑加载/成功/失败和证据标签；AI 对话只保留消息区、输入、发送和完成，首次发送才请求，后续固定快照并携带有限历史；两类结果都不写存储。
...
- AI 只在用户点按“组合分析”或在独立“AI 对话”中发送问题时使用 current-only USD 完整快照；只打开聊天、输入草稿或父页刷新都不调用。身份/账号/设备/存储内部/历史库/备份排除。逐只 AI 分类完整且标明推断，行业/角色占比由本机 Decimal 汇总；六维体检与聊天回答逐项绑定证据，数字本机重绘。会话不持久化；不展示伪造高级指标、外部归因、目标价、收益保证、预测或直接交易指令；
